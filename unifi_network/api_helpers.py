    ...
    **kwargs: Any
    ...
    while True:
        ...
        if response is None or response.status_code != 200:
            raise ValueError(f"API returned status {getattr(response, 'status_code', None)}")
    ...
    _LOGGER.debug("Fetched %d items across %d pages", len(all_items), (len(all_items) + page_size - 1) // page_size)
    return all_items
